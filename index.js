 ...
 */
bot.on(message(/кто я из наруто/gi), (ctx) => {
 ...
 * Genders
 */
bot.on(message(/какой я гендер/gi), (ctx) => {
 ...
 */
bot.on(message(/артом/gi), (ctx) => {
  console.log("артом");
  try {
    today = new Date();
    const artemIsBack = new Date(2021, 10, 10);
 ...
 */
bot.on(message(/пд/i), (ctx) => {
  console.log("пд");
  try {
    const scrape = function (callback) {
      let page = Math.floor(Math.random() * 600);
      const options = {
        method: "GET",
 ...
 * Say goodbye
 */
bot.on(message(/покеда/gi), (ctx) => ctx.reply("До свидания"));
bot.on(message(/до свидания/gi), (ctx) => ctx.reply("Покеда"));

/**
 * Quakoosha
 */
bot.on(message(/Quakoosha/gi), (ctx) =>
 ...
 * Marginal
 */
bot.on(message(/маргинал/gi), (ctx) =>
