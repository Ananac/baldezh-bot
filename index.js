...
bot.hears(/пд/i, ctx => {
  ...
  try {
    const scrape = function(callback) {
      let page = Math.floor(Math.random() * 691);
      const options = {
        method: "GET",
...
bot.hears(/stonks/gi, ctx => {
  try {
    ...
    cloudscraper(options).then(html => {
      let $ = cheerio.load(html);
      const usdForex = $(
        "body > table:nth-child(2) > tbody > tr:nth-child(2) > td > table > tbody > tr:nth-child(3) > td:nth-child(6) > table:nth-child(9) > tbody > tr:nth-child(3) > td > table > tbody > tr:nth-child(2) > td:nth-child(2)"
      );
      const euroForex = $(
